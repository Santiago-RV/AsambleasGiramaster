...
export default function VotingPage() {
  ...
  const { data: allPollsData, isLoading: isLoadingPolls, refetch: refetchPolls } = useQuery({
    ...
    queryFn: async () => {
      if (!meetingsData?.data || meetingsData.data.length === 0) return { polls: [] };
  ...
  const votedPollsHistory = allPollsData?.polls?.filter(poll => poll.has_voted === true) || [];

  const voteMutation = useMutation({
    mutationFn: async ({ pollId, voteData }) => await PollService.vote(pollId, voteData),
    onSuccess: async (data, variables) => {
      setVotedPolls(prev => new Set([...prev, variables.pollId]));
      setSelectedOptions(prev => { const newSelections = { ...prev }; delete newSelections[variables.pollId]; return newSelections; });
      setTextResponses(prev => { const newResponses = { ...prev }; delete newResponses[variables.pollId]; return newResponses; });
      setNumericResponses(prev => { const newResponses = { ...prev }; delete newResponses[variables.pollId]; return newResponses; });
      await Promise.all([refetchPolls(), queryClient.invalidateQueries({ queryKey: ['all-polls'] }), queryClient.invalidateQueries({ queryKey: ['meeting-polls'] })]);
      Swal.fire({ icon: 'success', title: '¡Voto Registrado!', text: 'Tu voto ha sido registrado exitosamente', timer: 2000, showConfirmButton: false });
    },
    onError: (error) => {
      console.error('Error al votar:', error);
      Swal.fire({ icon: 'error', title: 'Error al votar', text: error.response?.data?.message || 'Hubo un error al registrar tu voto. Intenta nuevamente.', confirmButtonColor: '#ef4444' });
    },
  });
  ...
  const handleOptionToggle = (pollId, optionId, pollType, maxSelections) => {
    setSelectedOptions(prev => {
      ...
      if (pollType === 'multiple') {
        if (currentSelections.includes(optionId)) return { ...prev, [pollId]: currentSelections.filter(id => id !== optionId) };
        if (currentSelections.length < (maxSelections || 999)) return { ...prev, [pollId]: [...currentSelections, optionId] };
        return prev;
      }
      return prev;
    });
  ...
  const handleVote = async (poll) => {
    ...
    if (poll.str_poll_type === 'text') {
      if (!textResponse.trim()) { Swal.fire({ icon: 'warning', title: 'Respuesta requerida', text: 'Debes ingresar una respuesta de texto', confirmButtonColor: '#3b82f6' }); return; }
      try { await voteMutation.mutateAsync({ pollId: poll.id, voteData: { str_response_text: textResponse.trim(), bln_is_abstention: false } }); } catch { /* Error manejado en onError */ }
      return;
    }

    if (poll.str_poll_type === 'numeric') {
      if (numericResponse === undefined || numericResponse === null || isNaN(numericResponse)) { Swal.fire({ icon: 'warning', title: 'Respuesta requerida', text: 'Debes ingresar un valor numérico', confirmButtonColor: '#3b82f6' }); return; }
      try { await voteMutation.mutateAsync({ pollId: poll.id, voteData: { dec_response_number: parseFloat(numericResponse), bln_is_abstention: false } }); } catch { /* Error manejado en onError */ }
      return;
    }

    if (selections.length === 0) { Swal.fire({ icon: 'warning', title: 'Selecciona una opción', text: 'Debes seleccionar al menos una opción para votar', confirmButtonColor: '#3b82f6' }); return; }

    try {
      if (poll.str_poll_type === 'single') await voteMutation.mutateAsync({ pollId: poll.id, voteData: { int_option_id: selections[0], bln_is_abstention: false } });
      else for (const optionId of selections) await voteMutation.mutateAsync({ pollId: poll.id, voteData: { int_option_id: optionId, bln_is_abstention: false } });
    ...
    <div className="space-y-6">
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-start gap-3">
        ...
        <div className="flex-1">
          <h3 className="font-semibold text-blue-900 mb-1">Sistema de Votaciones - Encuestas de Hoy</h3>
          <p className="text-sm text-blue-700">Aquí aparecerán las encuestas activas disponibles. Tu voto se registrará de forma segura y ponderada según tu coeficiente de propiedad.</p>
        </div>
        {votedPollsHistory.length > 0 && (
          <button onClick={() => setShowHistory(!showHistory)} className={`flex items-center gap-2 px-4 py-2 rounded-lg font-semibold transition-all ${showHistory ? 'bg-purple-600 text-white' : 'bg-white text-purple-600 border-2 border-purple-600 hover:bg-purple-50'}`}>
            <History size={20} />{showHistory ? 'Ocultar' : 'Ver'} Historial ({votedPollsHistory.length})
          </button>
        )}
      </div>
      ...
      {showHistory && votedPollsHistory.length > 0 && (
        <div className="space-y-4 bg-purple-50 border-2 border-purple-200 rounded-xl p-6">
          <h2 className="text-2xl font-bold text-purple-900 flex items-center gap-3"><Award className="text-purple-600" size={28} />Mis Votaciones ({votedPollsHistory.length})</h2>
          <p className="text-sm text-purple-700 mb-4">Historial completo de todas las encuestas en las que has participado</p>
          <div className="space-y-3">
            {votedPollsHistory.map((poll) => {
              const userVotes = poll.user_votes || [];
              return (
                <div key={poll.id} className="bg-white rounded-lg border-2 border-purple-200 p-5 hover:shadow-md transition-all">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2"><CheckCircle className="text-green-600 flex-shrink-0" size={24} /><h4 className="font-bold text-gray-900 text-lg">{poll.str_title}</h4></div>
                      {poll.str_description && <p className="text-gray-600 text-sm mb-3 ml-9">{poll.str_description}</p>}
                      {userVotes.length > 0 && (
                      ...
                      <div className="flex flex-wrap items-center gap-2 ml-9 text-sm">
                        <span className={`px-2 py-1 rounded-full text-xs font-semibold ${poll.str_poll_type === 'single' ? 'bg-blue-100 text-blue-700' : poll.str_poll_type === 'multiple' ? 'bg-indigo-100 text-indigo-700' : poll.str_poll_type === 'text' ? 'bg-amber-100 text-amber-700' : 'bg-cyan-100 text-cyan-700'}`}>
                          {poll.str_poll_type === 'single' ? 'Opción única' : poll.str_poll_type === 'multiple' ? 'Múltiple opción' : poll.str_poll_type === 'text' ? 'Texto libre' : 'Numérica'}
                        </span>
                        {poll.bln_is_anonymous && <span className="px-2 py-1 bg-gray-100 text-gray-700 rounded-full text-xs font-medium">Anónima</span>}
                      </div>
                    </div>
                    <div className="flex flex-col items-end gap-2"><div className="bg-green-500 text-white px-4 py-2 rounded-full flex items-center gap-2"><CheckCircle size={18} /><span className="font-bold text-sm">VOTADO</span></div></div>
                  </div>
                </div>
              );
            })}
      ...
      {activePolls.length > 0 ? (
        <div className="space-y-6">
          <h2 className="text-xl font-bold text-gray-800 flex items-center gap-2"><FileText className="text-green-600" size={24} />Encuestas Activas ({activePolls.length})</h2>
          {activePolls.map((poll) => {
            ...
            return (
              <div key={poll.id} className={`bg-white rounded-xl shadow-md border-2 overflow-hidden transition-all ${hasVoted ? 'border-green-300 bg-green-50/30' : 'border-blue-200 hover:shadow-lg'}`}>
                <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white p-6">
                  <div className="flex items-start justify-between">
                    <div className="flex-1"><h3 className="text-xl font-bold mb-2">{poll.str_title}</h3>{poll.str_description && <p className="text-blue-100 text-sm">{poll.str_description}</p>}</div>
                ...
                <div className="p-6">
                  {hasVoted ? (
                    <div className="text-center py-8"><CheckCircle className="w-16 h-16 text-green-600 mx-auto mb-4" /><h4 className="text-xl font-bold text-gray-800 mb-2">¡Voto Registrado!</h4><p className="text-gray-600">Tu voto ha sido registrado exitosamente para esta encuesta.</p></div>
                  ) : (
                    <>
                      <div className="mb-4">
                        ...
                        <>
                          <div className="space-y-3 mb-6">
                            {poll.options?.map((option) => {
                              ...
                              return (
                                <button key={option.id} onClick={() => handleOptionToggle(poll.id, option.id, poll.str_poll_type, poll.int_max_selections)} disabled={isVoting} className={`w-full text-left p-4 rounded-xl border-2 transition-all ${isSelected ? 'border-blue-600 bg-blue-50' : 'border-gray-200 hover:border-blue-300 hover:bg-gray-50'} ${isVoting ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}>
                                  <div className="flex items-center justify-between"><span className="font-medium text-gray-800">{option.str_option_text}</span>{isSelected && <CheckCircle className="text-blue-600" size={20} />}</div>
                                </button>
                              );
                            })}
                          </div>
        ...
        <div className="bg-white rounded-xl shadow-md p-12 text-center"><AlertCircle className="w-16 h-16 text-gray-400 mx-auto mb-4" /><h3 className="text-xl font-bold text-gray-800 mb-2">No hay encuestas activas</h3><p className="text-gray-600">No hay encuestas activas en este momento. Las encuestas aparecerán aquí cuando el administrador las inicie.</p></div>
      )}
